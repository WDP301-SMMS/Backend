...
import { MedicalInventoryModel, InventoryDetailModel } from '../models/medical.inventory.model';
...
try {
  HealthCheckTemplate = require('../models/healthcheck.template.model').HealthCheckTemplate;
} catch (e) {
...
const seedDatabase = async () => {
  try {
    ...
    await Promise.all([
      ...
      HealthCheckTemplate ? HealthCheckTemplate.deleteMany({}) : Promise.resolve(),
    ]);
    ...
    console.log(`Seeded ${createdUsers.length} users, ${createdClasses.length} classes, ${students.length} students.`);
    ...
    const template = {
        ...
        checkupItems: [
            { itemId: new mongoose.Types.ObjectId(), itemName: 'Chiều cao', unit: 'cm', dataType: 'NUMBER' },
            { itemId: new mongoose.Types.ObjectId(), itemName: 'Cân nặng', unit: 'kg', dataType: 'NUMBER' },
            { itemId: new mongoose.Types.ObjectId(), itemName: 'Thị lực', unit: '/10', dataType: 'TEXT' },
            { itemId: new mongoose.Types.ObjectId(), itemName: 'BMI', dataType: 'NUMBER' },
            { itemId: new mongoose.Types.ObjectId(), itemName: 'Sâu răng', unit: 'cái', dataType: 'NUMBER' },
        ]
    };
    const createdTemplate = HealthCheckTemplate ? await HealthCheckTemplate.create(template) : template;
    ...
    const campaigns = await HealthCheckCampaign.insertMany([
        {
            ...
            templateId: createdTemplate.itemId ? createdTemplate._id : new mongoose.Types.ObjectId(),
    ...
    for (const student of studentsForConsent) {
        const status = Math.random() > 0.1 ? ConsentStatus.APPROVED : ConsentStatus.DECLINED; // 90% đồng ý
        consents.push({
            campaignId: latestCampaign._id,
            studentId: student._id, // GIẢ ĐỊNH QUAN TRỌNG: model của bạn đã sửa ref thành 'Student'
            parentId: student.parentId,
            status,
            reasonForDeclining: status === ConsentStatus.DECLINED ? 'Lý do gia đình' : null,
            confirmedAt: new Date(),
        });
    }
    await HealthCheckConsent.insertMany(consents);
    const approvedStudents = consents.filter(c => c.status === ConsentStatus.APPROVED).map(c => c.studentId);
    
    // Tạo Health Check Results cho các học sinh đã đồng ý
    const results = [];
    const checkupItemIds = {
        height: (createdTemplate.checkupItems.find((item: ICheckupItem) => item.itemName === 'Chiều cao')!).itemId,
        weight: (createdTemplate.checkupItems.find((item: ICheckupItem) => item.itemName === 'Cân nặng')!).itemId,
        vision: (createdTemplate.checkupItems.find((item: ICheckupItem) => item.itemName === 'Thị lực')!).itemId,
        bmi: (createdTemplate.checkupItems.find((item: ICheckupItem) => item.itemName === 'BMI')!).itemId,
        dental: (createdTemplate.checkupItems.find((item: ICheckupItem) => item.itemName === 'Sâu răng')!).itemId,
    };
    ...
    for (const studentId of approvedStudents) {
        ...
        results.push({
            campaignId: latestCampaign._id,
            studentId: studentId,
            checkupDate: faker.date.between({ from: latestCampaign.startDate, to: latestCampaign.endDate }),
            resultsData: [
                { itemId: checkupItemIds.height, itemName: 'Chiều cao', value: height, unit: 'cm' },
                { itemId: checkupItemIds.weight, itemName: 'Cân nặng', value: weight, unit: 'kg' },
                { itemId: checkupItemIds.vision, itemName: 'Thị lực', value: hasBadVision ? '7/10' : '10/10', unit: '/10', isAbnormal: hasBadVision },
                { itemId: checkupItemIds.bmi, itemName: 'BMI', value: bmi, isAbnormal: isObese },
                { itemId: checkupItemIds.dental, itemName: 'Sâu răng', value: hasCavity ? 1 : 0, unit: 'cái', isAbnormal: hasCavity }
            ],
            overallConclusion: isObese || hasBadVision ? 'Cần theo dõi' : 'Bình thường',
