...
const getPushContent = (type: NotificationType, data: RelatedData): { title: string; body: string } => {
  switch (type) {
    case NotificationType.HEALTH_CHECK_CAMPAIGN_NEW:
      return { title: 'Khám Sức Khỏe', body: `Nhà trường vừa công bố chiến dịch mới: ${data.campaignName || 'Chiến dịch khám sức khỏe'}.` };
    case NotificationType.HEALTH_CHECK_RESULT_READY:
      return { title: 'Kết Quả Sức Khỏe', body: 'Kết quả khám sức khỏe của con bạn đã có.' };
    case NotificationType.VACCINE_CAMPAIGN_NEW:
      return { title: 'Tiêm Chủng', body: `Nhà trường vừa công bố chiến dịch mới: ${data.campaignName || 'Chiến dịch tiêm chủng'}.` };
    default:
      return { title: 'Thông Báo Mới', body: 'Bạn có một thông báo mới từ School Health.' };
  }
};
