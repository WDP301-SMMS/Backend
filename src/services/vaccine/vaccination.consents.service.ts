...
public async respondByStudentAndCampaign(
    ...
    const consent = await VaccinationConsentModel.findOne({ campaignId, studentId });
    
    if (!consent) {
        const error: AppError = new Error('Consent request not found for this student in this campaign.');
        error.status = 404;
        throw error;
    }

    // Bước 2: Kiểm tra quyền sở hữu (bảo mật)
    const student = await StudentModel.findOne({ _id: studentId, parentId: parentId });
    if (!student) {
        const error: AppError = new Error('You are not authorized to respond for this student.');
        error.status = 403;
        throw error;
    }
    if (!consent.parentId.equals(parentId)) {
        const error: AppError = new Error('Mismatched parent information. Access denied.');
        error.status = 403;
        throw error;
    }
    ...
    try {
        ...
        await VaccinationCampaignModel.updateOne({ _id: campaignId }, summaryUpdate);

        return consent;

    } catch (error) {
        console.error("INCONSISTENCY WARNING: An error occurred during a non-transactional update.", error);
        throw error; 
    }
}
