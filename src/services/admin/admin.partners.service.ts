...
class AdminPartnerService {
  ...
  public async createPartner(
    partnerData: ICreatePartnerAndManagerDTO,
  ): Promise<IHealthcareOrganization> {
    const existingPartner = await this.partners.findOne({
      $or: [
        { name: partnerData.organization.name },
        { email: partnerData.organization.email },
      ],
    });
    if (existingPartner) {
      throw createAppError(
        409,
        `A partner with this name or email already exists.`,
      );
    }
  ...
  public async addStaffToPartner(
    partnerId: string,
    staffData: IAddStaffDTO,
  ): Promise<IPartnerStaff> {
  ...
  public async updateStaffMember(
    staffId: string,
    updateData: Partial<IAddStaffDTO>,
  ): Promise<IPartnerStaff> {
    const updatedStaff = await this.staffs.findByIdAndUpdate(
      staffId,
      updateData,
      { new: true },
    );
  ...
  public async removeStaffFromPartner(
    partnerId: string,
    staffId: string,
  ): Promise<void> {
    const existingRecord = await this.vaccinationRecords.findOne({
    ...
    try {
      ...
      const isStaffInPartner = partner.staffMembers.some(
        (memberId) => memberId.toString() === staffId,
      );
      ...
      if (!(error instanceof Error && 'status' in error)) {
        throw createAppError(
          500,
          `Failed to remove staff member: ${(error as Error).message}`,
        );
      }
      throw error;
    } 
  ...
  public async updateManager(
    managerId: string,
    updateData: Partial<Omit<IManagerInfo, '_id' | 'organizationId'>>,
  ): Promise<IManagerInfo> {
    const updatedManager = await this.managers.findByIdAndUpdate(
      managerId,
      updateData,
      { new: true },
    );
  ...
  public async replaceManager(
    partnerId: string,
    newManagerData: Omit<IManagerInfo, '_id' | 'organizationId'>,
  ): Promise<IManagerInfo> {
    const partner = await this.partners.findById(partnerId);
    if (!partner) {
  ...
  public async getPartners(query: {
    ...
    partners: IHealthcareOrganization[];
    total: number;
    pages: number;
    currentPage: number;
  }> {
  ...
  public async updatePartner(
    partnerId: string,
    updateData: Partial<IHealthcareOrganization>,
  ): Promise<IHealthcareOrganization> {
    const updatedPartner = await this.partners.findByIdAndUpdate(
      partnerId,
      { $set: updateData },
      { new: true },
    );
  ...
  public async updatePartnerStatus(
    ...
    if (partner.isActive === isActive) {
      throw createAppError(
        409,
        `Partner is already ${isActive ? 'active' : 'inactive'}.`,
      );
    }
